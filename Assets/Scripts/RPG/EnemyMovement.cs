...
public class EnemyMovement : GradientHealth
{
    ...
    public bool isDead;
    public float distanceToPoint, changeWaypointWhenThisClose, stopFromPlayer, turnSpeed;
    ...
    void Patrol()
    {
        if (waypoints.Length <= 0 
            || Vector3.Distance(player.position, transform.position) <= sightRange 
            || isDead)
        {
            return;
        }
    ...
    void Seek()
    {
        if(Vector3.Distance(player.position, transform.position) > sightRange 
            || Vector3.Distance(player.position, transform.position) < attackRange 
            || isDead)
        {
            return;
        }
